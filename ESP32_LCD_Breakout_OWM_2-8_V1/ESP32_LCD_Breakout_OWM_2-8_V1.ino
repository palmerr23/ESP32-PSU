// Demo for SC WiFi Enabled ESP32 LCD Breakout - 2.8" version
...
#define SCREENROT 3
#define TOUCHROT 1 // or 3 for some touch screens
#define F1
#define F2

#ifdef F1
  #include "arial.h"
  #define GUI_FONT arial_font
  #define GWID (0)
#else
  #include "bigfont.h"
  #define GUI_FONT BigFont
  #define GWID (-6)
#endif

#ifdef F2
  #include "small_font.h"
  #define SFONT myke_font
#else
  #include "medfont.h"
  #define SFONT MedFont
#endif
#include "lcd.h"
#include "gui.h"
#include "graphics.h"
...
void setup() {
  ...
  while(!Serial && (millis() < 2000))
    ;
   Serial.printf("Screen width %i, height %i\n", width, height);
  ...
  //display
  displaySetup();
  setrotation(SCREENROT);     //change this and image will adjust - also see displaySetup() in LCD.H
...
void getWebData(){
  ...
  if(current_icon != NULL){
   eightBitBitmap(2,50,current_icon);
  }
  //main
  char main_desc[15]="";      //we'll use up to around 12 characters, 1-12
  getChar("\"main\":",&payload,main_desc,14);
  padFix(main_desc+1,12);
  characterarray(110,45,main_desc+1,GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
  
  //full description
  char full_desc[40]="";      //
  getChar("\"description\":",&payload,full_desc,40);
  padFix(full_desc+1,36);
  characterarray(110,68,full_desc+1,GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,SFONT);
  
  //now temp
  characterarray(108,80,"Now ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
  drawTemp(165,80,temp);
  //min temp
  characterarray(110,100,"Min ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
  drawTemp(165,100,temp_min);
  //max temp
  characterarray(110,120,"Max ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
  drawTemp(165,120,temp_max);
  //long


  //other
 // eightBitBitmap(5,125,icon_sunrise);  
  characterarray(5,145,"Sunrise ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
 // eightBitBitmap(5,150,icon_sunset);
  characterarray(5,165,"Sunset ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
 // characterarray(10,185,"Updated: ",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
  drawTime(140,145,sunrise,GUI_FOREGROUND_COLOUR);
  drawTime(140,165,sunset,GUI_FOREGROUND_COLOUR);
 // drawTime(150,185,UTC_time,GUI_FOREGROUND_COLOUR);
...
void mainScreen(){
  ...
  char ipAdd[20]="Test";
  IPAddress ip;
  guiObject setupButton={235,180,80,30,"WiFi",0,0,1,1,BUTTON};  //x,y,w,h,text,value,pressed,visible,enabled,type
  ...
  while(!done){
    ...
    if(WiFi.status()==WL_CONNECTED){
      ...
      monoBitmap(260,140,GREEN,GUI_BACKGROUND_COLOUR,wifi);
      ...
      monoBitmap(260,140,RED,GUI_BACKGROUND_COLOUR,wifi);      
      strCopy("---.---.---.--- ",ipAdd,17);
    }
    characterarray(200,220,ipAdd,GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,SFONT);
    if(UTCzero&&(TZoffset!=-1)){      //UTCzero <=> time has been set from NTP at some point, TZ has been set
      drawTimeSeconds(5,215,UTCzero+millis()/1000,(NTPstatus?GUI_FOREGROUND_COLOUR:GUI_DISABLE_COLOUR));      
    }else{
      characterarray(5,215,"--:--:--",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
    }
    characterarray(5,190,"Local time",GUI_FOREGROUND_COLOUR,GUI_BACKGROUND_COLOUR,GUI_FONT);
...
int pickWifi(char* a, int n){ //destination, max length
  boolean done=0;
  guiObject cancelButton={218,2,100,26,"Cancel",0,0,1,1,BUTTON};  //x,y,w,h,text,value,pressed,visible,enabled,type  
  char ssidList[6][36];
  guiObject ssidButton[6]={
