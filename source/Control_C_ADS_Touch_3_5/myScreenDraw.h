...
short tMode(short sv){
	...
	if (sv == TBUT)
	{
	  if (pSetA.trackOn)
	  {
		if(pSetA.eTrack <= 1.0 - SMALL_DIFF)
			return BBACT;
		else
			return BBINACT;
	  }
...
void drawIndicators(short aMenu)
{
	...
	// wireless connected
	//tft.fillRoundRect(VALX+40, 0, ABUT, ABUT,  ABUTR, (pSetA.WifiConnected)?IENABLE:ITRIG); 
	tft.setCursor(sx(READX + 60), sy(TOPLEGS));
	tft.setTextColor((pSetA.WifiConnected)?IND_ON:IND_OFFC);
	tft.print("W");
	...
	tft.setCursor(sx(READX + 75), sy(15));
	tft.print("[E]");
  ...
  switch (aMenu)
  {
    case 0:		// main menu
		...
		tft.fillRoundRect(sx(VALX+35), sy(6), sx(8), sx(8),  3, xcol); 
 ...
 // for (short i =0; i< NUMLABELS; i++)
  {
    if(readVals[readCntr].menu == currentMenu)
	{
      ...
      printfloatX(readVals[readCntr].vp, readVals[readCntr].units, readVals[readCntr].font, readVals[readCntr].textSize, readVals[readCntr].pre, readVals[readCntr].post, butDigit, readVals[readCntr].xpos,  readVals[readCntr].ypos,  readVals[readCntr].fmt, tempClr, tempClr, BGCOL);
	}
    readCntr++;
  }
  /*********** debug
  ...
  while (setVals[i].vp)//(short i = 0; i < NUMSETTINGS; i++)
  {
    if(setVals[i].menu == currentMenu) 
    {
      if (i == selected){
        ...
        if (all)
          printfloatX(setVals[i].vp, setVals[i].units, setVals[i].font, setVals[i].textSize, setVals[i].pre, setVals[i].post, highDigit, setVals[i].xpos,  setVals[i].ypos, setVals[i].fmt, setVals[i].clr, setVals[i].clr, BGCOL);
    }
    i++;
  }
}
...
//Serial.print("Decimal ");Serial.println(decimal);
  ...
  switch (fmt){
	...
	case 'Q' :  // boolean print the first char in v2 string for T second for F
	  i = (*(bool*)vp) ? 0 : 1;
//	  Serial.printf("X: %i %i %i %i %i\n", i, pSetA.trackSv, pSetA.trackSa, vTrack, cTrack);
      tft.setTextColor(txtcol);
	  tft.print((char*)(v2 + i));
